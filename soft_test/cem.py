...
class CEM():
    ...
    def __init__(self, model, K, T, us, ls, ua, la, u_init = 0, base=False):
        self.K = K  # N_SAMPLES
        self.T = T  # TIMESTEPS
        self.u_lower_bound = la
        self.u_upper_bound = ua
        self.s_lower_bound = ls
        self.s_upper_bound = us
...
#
    ...
    def predict(self, x, target):
        self.target = target
        return np.array(self.control(x),dtype=np.float32)
